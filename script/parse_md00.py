...
def updateJson(json_item, modified):
    ...
    if modified == 'D':
        for item in input_json:
            if parseLink(stripToRunbookName(json_item['link'])) == parseLink(
...
'''
    ...
    write_runbook_metadata() = Used to update Runbook with metadata (only) used
        for Runbooks that do not have metadata in them
'''
...
def process_metadata(runbook, modified, reader, runbook_name, thisyaml):
    try:
...
def process(runbook_name, runbook_json, modified):
    ...
    # MOVING INTO METADATA RUNBOOK section
    # READ OUTPUT JSON FILE INTO 'RUNBOOK'
    output_json_file = sys.argv[3]
    with open(output_json_file, mode='r') as out:
        runbook = json.load(out)
    out.close()

    if 'type' in oldyamldata:
        # newdict is used to store metadata for current runbook
        # check metadata in Runbook is valid:
        if not checkkeys(oldyamldata):
            print 'Error in Keys in this Runbook'
            sys.exit(1)
        # If keys are not correct
    process_metadata(runbook, modified, reader, runbook_name, oldyamldata)
    return
